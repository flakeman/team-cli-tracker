...
## Environment
- Main execution node: `test.abuztech.ru:22221`
- Observer nodes: `test.abuztech.ru:22222`, `test.abuztech.ru:22223`
- Test API port: `:4111`
- Binary: freshly built from current `main` (`node-linux-fulltest` uploaded to `22221`).

## Main Matrix Result (22221)
...
Conclusion for 22221: PASS.

## 3-VPS Observer Check (22222/22223)
- From `22222` and `22223`:
  - direct `curl` to `http://test.abuztech.ru:4111/healthz` -> connection refused (`000`)
  - board peer sync to `http://test.abuztech.ru:4111` -> sync clock error / empty board

Conclusion for cross-node e2e: BLOCKED by network reachability (port `4111` inaccessible from other VPS).

## Blocking Issue
- `4111/tcp` is not reachable from `22222` and `22223` to `22221`.
...
2. Re-run the same matrix with peer sync enabled from `22222` and `22223`.
